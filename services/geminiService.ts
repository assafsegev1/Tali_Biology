...
if (!apiKey) {
  console.warn("Warning: API Key is missing. AI features will not work.");
}
...
export const evaluateAnswer = async (
  ...
  if (!apiKey) return { score: 0, feedback: "שגיאה: מפתח API חסר. נא להגדיר VITE_API_KEY ב-Vercel או לוודא ש-process.env.API_KEY קיים." };
  ...
  try {
    ...
    return {
      score: 0,
      feedback: "אירעה שגיאה בעת בדיקת התשובה. אנא נסו שנית."
    };
  }
};

export const getTopicSummary = async (topic: string): Promise<string> => {
  if (!apiKey) return "שגיאה: מפתח API חסר.";
